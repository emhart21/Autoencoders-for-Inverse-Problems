...
%%
target_train_noisy = zeros(n_train,n,n);
target_test_noisy = zeros(n_test,n,n);
input_train = zeros(n_train,p,length(angles));
input_test = zeros(n_test,p,length(angles));

for k = 1:n_train
    target_train_noisy(k,:,:) = X_train{1,k} + WhiteNoise(X_train{1,k},noiseLevel);
    input_train(k,:,:) = B_train{1,k};
end
for k = 1:n_test
    target_test_noisy(k,:,:) = X_test{1,k} + WhiteNoise(X_test{1,k},noiseLevel);
...
target_test_noisy = target_test_noisy - min(target_test_noisy(:));
target_test_noisy = target_test_noisy / max(target_test_noisy(:));
target_train_noisy = target_train_noisy - min(target_train_noisy(:));
target_train_noisy = target_train_noisy / max(target_train_noisy(:));
...
save('SheppLoganData10000noisy.mat','input_test','target_test_noisy','input_train','target_train_noisy', 'input_test_48', 'input_train_48')
